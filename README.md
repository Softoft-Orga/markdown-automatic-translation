...
```
Quick start
Initialize the editable prompt (creates ~/.mdxlate/translation_instruction.txt):
...
Behavior
Prompt: default lives at ~/.mdxlate/translation_instruction.txt (created by mdx init).
You can edit it freely or pass --prompt-path.
...
from mdxlate.cli import start_translation  # thin wrapper you ship
...
start_translation(
    ...
    api_key=None,       # falls back to env via your CLI, here pass explicitly if needed
    base_url=None,
    prompt_path=None,
)
Files of interest
...
# Optional: pytest defaults
[tool.pytest.ini_options]
addopts = "-q"
testpaths = ["tests"]
Make sure your layout matches
bash
Copy code
repo/
  pyproject.toml
  ...
  main.py           # your local test runner (optional)
That’s it. If you want, we can add a short section to README explaining the .mdxlate.hashes.json logic in one line, or leave it as is to keep things tight.